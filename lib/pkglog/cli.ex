defmodule Pkglog.CLI do
  ...
  def process_options({opts, argv}) do
    ...
    config = %{
      ...
      pacman_actions: opts[:pacman_actions] || false,
  ...
  defp print_help do
    IO.puts(
      IO.ANSI.format([
        ...
        "usage: pkglog ",
        yellow(),
        "[-h] [-u | -i | -I | -n] [-e] [-P] [-d DAYS] [-a] [-b] [-j] [-v] [-c]",
        ...
        IO.ANSI.format_fragment([yellow(), "  -P, --pacman-actions"], true),
        "     show actions performed by pacman commands; packages pulled in as\n",
        "                            dependencies are not shown, installs done with\n",
        "                            --asdeps are shown as \"installed as dependency\"\n",
